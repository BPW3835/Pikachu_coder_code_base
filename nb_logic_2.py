...
class ExecuteValidation:
    ...
    async def process_emails(self):
        ...
        try:
            neverbounce_results_extender_list = []
            merge_json_structure_rows_extender_list = [] 
            ...
            for chunk_id, chunk in self.bq_logic.fetch_unvalidated_emails(sql, self.max_batch_size):
                ...
                if neverbounce_emails:
                    neverbounce_results_extender_list.extend(neverbounce_results)
                    self.logger.info(f'neverbounce_results_extender_list - {len(neverbounce_results_extender_list)}')
                self.bq_logic.insert_in_lookup(self.project_id, self.lookup_dataset, self.lookup_table, neverbounce_results)
                merge_json_structure_iterator = self.bq_logic.structure_merge_source(self.project_id,self.structure_merge_udf_path,merge_source_rows)
                merge_json_structure_rows = list(merge_json_structure_iterator)
                merge_json_structure_rows_extender_list.extend(merge_json_structure_rows)
                self.logger.info(f'current data in merge_json_structure_rows_extender_list is  - {len(merge_json_structure_rows_extender_list)}')
                if  len(merge_json_structure_rows_extender_list) >= 200000  and len(neverbounce_results_extender_list) >= 5000:
                    self.bq_logic.merge_into_hmz_tables(self.project_id, list_of_tables, merge_json_structure_rows_extender_list)
                    self.logger.info(f'process completed for {len(merge_json_structure_rows_extender_list)} rows')
                    neverbounce_results_extender_list = []
                    merge_json_structure_rows_extender_list = []
